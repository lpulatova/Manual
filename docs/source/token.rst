...
После заполнения пользовательской токенизации возвращаемся на форму «Справочник объектов» и на панели инструментов нажимаем на кнопку «Выгрузить в Excel при помощи Smart Api» |Excel|

.. image:: _static/screens/token05.png

На открывшейся форме «Параметры выгрузки через Smart Api» выбираем вид выгрузки «**Обучить модель**» и тип токенизации «**Пользовательская токенизация**». Затем нажимаем на кнопку Отправить данные |Отправить данные|.

.. image:: _static/screens/token06.png
...
Для контроля эффективности выполненных работ разработан новый сервис «Получить метрики» - подсчет метрик по распознаванию значений ОД в классе. Запускается через новое меню “Обучить модель” (открывается с формы “Справочник объектов”).
